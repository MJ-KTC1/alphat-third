...
\n";
        ...
        if(txt === 'สวัสดีแก๊งกระล่อนยัง''สวัสดี''555''หำ''แตด''ดีคับ''ประเทศกูมี''งงดิสาส' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        }else if(txt === 'สวัสดีแก๊งกระล่อนยัง''สวัสดี''555''หำ''แตด''ดีคับ''ประเทศกูมี''งงดิสาส' && !isAdminOrBot(seq.from_) && seq.toType == 2){this._sendMessage(seq,"Not permitted !");}
